<script>
   function handleSendComment(buttonElement) {
       ...
       const inputOL = document.querySelector(`#inpbalassaja-${targetid}`);
       const commentses1 = document.querySelector(`#inpbalassajahidden-${targetid}`);
       const fileInput1 = document.querySelector(`#filepicker1-${targetid}`);
       const clearBtn1 = document.querySelector(`#close-dibales-${targetid}`);
       const delprev1 = document.querySelector(`#image-preview1-${targetid}`);
       ...
       if (hasImage) {
           ...
           .catch(error => {
               if (error.itsToxic) {
                   const brcmt = document.querySelector(`#divbrcmt2-${error.commentartiestoriesid}`);
                   const alertToxic = document.querySelector(`#Alert-Toxic1-${error.commentartiestoriesid}`);
           ...
           .catch(error => {
               if (error.itsToxic) {
                   const brcmt = document.querySelector(`#divbrcmt2-${data.commentartiestoriesid}`)
                   const alertToxic = document.querySelector(`#Alert-Toxic1-${data.commentartiestoriesid}`);
                   brcmt.classList.remove('hidden');
                   alertToxic.innerText = 'Kamu kasar sekali!';
                   setTimeout(() => {
                       alertToxic.innerText = '';
   ...
   document.addEventListener('focusin', function(event) {
       ...
       inputOL.addEventListener('keydown', (e) => {
           if (e.key === 'Enter' && !e.shiftKey) {
               e.preventDefault();
               const sendButton = document.querySelector(`#btnimg-sendcom-${targetid}`);
               if (sendButton) {
                   handleSendComment(sendButton);
               }
           } else {
